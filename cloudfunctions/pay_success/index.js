...
exports.main = async (event, context) => {
  ...
  try {
    // 事务内不支持 where 查询，充值用户查询放到事务外执行
    let rechargeUser = null
    const preOrderRes = await db.collection('order').doc(orderId).get()
    const preOrder = preOrderRes.data

    if (preOrder && preOrder.type === 'recharge') {
      const userRes = await db.collection('user').where({
        _openid: preOrder._openid
      }).get()
      if (userRes.data && userRes.data.length > 0) {
        rechargeUser = userRes.data[0]
      }
    }
    ...
    const result = await db.runTransaction(async transaction => {
      ...
      // 4. 如果是充值订单，处理余额（事务内只做 doc 单记录原子更新）
      if (order.type === 'recharge' && rechargeUser) {
        const totalGet = order.totalGet || (order.amount + order.giveAmount)

        await transaction.collection('user').doc(rechargeUser._id).update({
          data: {
            balance: db.command.inc(totalGet)
          }
        })
      }
